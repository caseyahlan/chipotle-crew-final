...
server <- function(input, output) {
  ...
  #############################
        ...
        vote.choose <- reactive({
          ...
          colnames(voters) <- c("party", "first name", "last name", "state", "vote")
          return(voters)
        })
  ...
  #############################
        ...
        output$house.missed <- renderPlotly({
          ...
          if (input$order == "alphabetically") {
            ...
            house.members$name<- factor(house.members$name, levels = house.members$name[order(house.members$percent, decreasing = TRUE)])
          }  
        ...
        output$senate.ex <- renderText({
          return("It appears that some Congresses had more than 100 members because all members that served during the Congress are shown, 
                 even though no more than 100 members were active at the same time.")
        })
        ...
        output$senate.with <- renderPlotly({
          ...
          senate.members$party <- as.factor(unlist(senate.members$party))
          if (input$order == "alphabetically") {
            senate.members$name<- factor(senate.members$name, 
                                         levels = senate.members$name[order(senate.members$last_name)])
          } else if (input$order == "increasing") {
            senate.members$name<- factor(senate.members$name, 
